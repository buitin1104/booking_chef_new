...
var addPlaceholder = null,
    countImage = 0,
    input_name_multiple = null;
...
$("#use-article-sample").on("click", function (event) {
    $.ajax({
        ...
        url:
            BASE_API_URL +
            "/article_samples/" +
            $("input[name='article-sample']:checked").val(),
        beforeSend: function () {
            $(".loadding-body").show();
...
var settings = {
    ...
    onSuccess: function (files, response, xhr) {
        ...
        if (response.code == 200) {
            if (mediaAvatar) {
                jQuery("input[name='check_image_media[]']:checked").each(
                    function () {
                        jQuery(this).prop("checked", false);
                    }
                );
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.code == 200) {
                        jQuery(
                            "input[name='check_image_media[]']:checked"
                        ).each(function () {
                            var id = jQuery(this)
                                .parent("li")
                                .children("#value-id")
    ...
    $.ajax({
        ...
        beforeSend: function () {
            $("#list-media .loadding-media").html(
                '<img src="/images/loading-1.gif" width="40px"> Loading'
            );
...
var getArticleSample = function () {
    $.ajax({
        ...
        beforeSend: function () {
            $("#list-article").html(
                '<div class="loadding-media"><img src="/images/loading-1.gif" width="40px"> Loading</div>'
            );
        },
...
function checkDivImageEmpty() {
    let modal = $(".upload-list-img").closest(".modal");

    if (countImage == 0) {
        let html = `<div class="text-center img-preview img-preview-placeholder" onclick="initMediaDiv('uploadListProductVariations', 'previewPlaceholder', 'product_variation_images')">
                        <img src="/images/placeholder.png" class="img-thumbnail img-thumbnail-placeholder">
                    </div>`;
        modal.find(".upload-list-img").parent().prepend(html);
    }
}
